<?php
/**
 * Direct access script to show logs (bypasses router)
 * Access: https://data-q.org/api/direct-show-logs.php
 */
...
<head>
    ...
    <style>
        ...
        .highlight { background: #fff3cd; padding: 2px; }
    </style>
</head>
<body>
    <div class="container">
        ...
        $possibleLogPaths = [
            ...
            '/home/dataqdxe/public_html/error_log',
            '/home/dataqdxe/logs/error_log',
        ];
        ...
        if (!empty($foundLogs)) {
            ...
            echo '<h3>Recent Error Log Entries (Last 100 lines from first found log):</h3>';
            $logFile = $foundLogs[0];
            ...
            foreach ($recentLines as $line) {
                ...
                if (stripos($line, 'smtp') !== false || stripos($line, 'email') !== false || 
                    ...
                    stripos($line, 'ehlo') !== false || stripos($line, 'authentication') !== false) {
                    echo "<span class='highlight'>$line</span>";
                } else {
                    echo $line;
                }
            }
            ...
            echo '<h3>⚠️ No Error Logs Found in Common Locations</h3>';
            echo '<p>For Afrihost/cPanel hosting, try:</p>';
            echo '<ul>';
